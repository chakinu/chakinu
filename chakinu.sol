/**
                                          ...
                                          Socio-Experiment 
                                     (Decentralized Might Work!)
                                     Website: https://chakinu.com
 
                                          #CHAK Community
                                          
                                     https://t.me/chakinu
                                     https://reddit.com/r/chakinu
                                     https://twitter.com/chakinuofficial
                                     
                                  ---THE ALDOGE KILLER - #CHAKARMY---
                                  

                                   << $CHAK Almighty Tokenomics >>

  Total Supply  : 1,000,000,000,000,000 CHAK
  Decimals      : 18
  
  DECENTRALIZED : 50% Locked at LP and Key Thown as NFT to be found by the next Community!
  WOOFMEISTER   : 50% Supply is burned at start sent to vB's burning wallet!
  RUG-PROOF     : 5% liquidity decision burn once/monthly BNB^CHAK by new Key Founder.
  ANTI-SHARK    : 5% burned in every transaction forever!
  HODL-WOOF     : 5% of every transactions distributed to hodlers.
  CHAKINESS     : 5% of the fee is added to LP when transacting.
 
   
                               --- DECENTRALIZAED PERPETUAL COMMUNITY ---
 */

// SPDX-License-Identifier: Humanity Licensed
pragma solidity >=0.6.0 <0.8.0;
    ...
    /**
     * The @dev Returns amount of tokens owned by `account`.
     */
    function balanceOf(address account) external view returns (uint256);

    /**
     * The @dev Moves `amount` tokens from the caller's account to `recipient`.
     *
...
contract CHAKINU is Context, IERC20, Ownable {
...
// Special Feature
// ANTI-SHARK - FEE BURNING
// 5% transaction fee is deducted as well which is not reflected (distributed), 
// It is simply burned. The same as liquidityFee mechanism.
// Tokens are sent to this contract and only burned once if numTokensSellToAddToLiquidity is reached.
uint256 public _burnFee = 5; // percentage of liquidityFee that is burned
...
// Special feature
// RUG-PROOF - LIQUIDITY BURNING
// All liquidity tokens are send to this contract.
// Once monthly owner can decide to do a liquidity burn.
// The maxLiquidityBurnPercentage will then be renounced from liquidity.
// BNB is swapped to CHAK. Finally all this CHAK's are burned.
...
uint256 public lastLiquidityBurn = 0;
    ...
    IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
     // Create a uniswap pair for this new token (CHAK/WBNB)
...
// Removes amount LPTokens from the contract, exchanging them for WBNB and CHAK
// The WBNB is exchanged for CHAK and finally all generated CHAK are burned.
function removeLiquidityAndBurn(uint256 amount) public onlyOwner() {
    ...
    uint256 prevCHAKBal = balanceOf(address(this));
    ...
    uint256 CHAKBal = balanceOf(address(this));
    uint256 toBurn = CHAKBal.sub(prevCHAKBal);
...
function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
    ...
    //Change Pancakeswap Router Anytime!
	//FreezyEx Fix
